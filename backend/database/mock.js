module.exports = {
  users: [
    {
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      name: 'Luiz Henrique',
      email: 'luiz@mail.com',
      password: '1234',
      apiKey: ['fcd5015c-10d3-4e9c-b395-ec7ed8850165'],
      createdAt: new Date('1997-01-14').getTime()
    }
  ],
  feedbacks: [
    {
      text: 'Muito bom!',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'OTHER',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-11-13').getTime()
    },
    {
      text: 'Muitos erros, putz kkkkkkk',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'ISSUE',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-10-23').getTime()
    },
    {
      text: 'Podia ter um botão de solicitar demo',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'IDEA',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-09-23').getTime()
    },
    {
      text: 'Podia ter um botão de solicitar demo 1',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'IDEA',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-12-23').getTime()
    },
    {
      text: 'Podia ter um botão de solicitar demo 2',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'IDEA',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-08-23').getTime()
    },
    {
      text: 'Muitos erros, putz kkkkkkk 2',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'ISSUE',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-05-23').getTime()
    },
    {
      text: 'Tava bom, agora parece que piorou',
      fingerprint: '490135491',
      id: 'eab759f8-f238-4ff9-ae91-ee1558982329',
      apiKey: 'fcd5015c-10d3-4e9c-b395-ec7ed8850165',
      type: 'ISSUE',
      device: 'Chrome 85.0, macOS 10.14',
      page: 'https://feedbacker.com/pricing',
      createdAt: new Date('2021-05-23').getTime()
    }
  ]
}
